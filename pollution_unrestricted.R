...
C1yield2<- read.csv("DataY.csv")
C2yield2<-read.csv("DataX.csv")
C1yield <- C1yield2[,-1]
rownames(C1yield) <-  C1yield2[,1]
colnames(C1yield) <- seq(0,23)
C1yield<-data.matrix(C1yield)

C2yield <-  C2yield2[,-1]
rownames(C2yield) <-  C2yield2[,1]
colnames(C2yield) <- seq(1,3)
C2yield<-data.matrix(C2yield)

save(C1yield, file = "DataY.RData")
save(C2yield, file = "DataX.RData")
# Some data specs:
startDate = as.Date("2015-01-01")
endDateEstimate = as.Date("2019-12-31")
endDateForecast = as.Date("2020-06-30")
############################################################
...
load('DataY.RData')
load('DataX.RData')
Yall = C1yield; tauAllY = as.numeric(colnames(Yall)); datesY = as.Date(rownames(Yall));
Xall = C2yield; tauAllX = as.numeric(colnames(Xall)); datesX = as.Date(paste(rownames(Xall)))

niter =1
compTimesAll = JeAll = rhoAll = numeric(niter) 
if(selectLagP) propSelectedAll = matrix(0, nr=niter, nc=pMax)
...
useIndsForeY = which(1.0*(datesY >= startDate)*(datesY <= endDateForecast)*(rowSums(!is.na(Yall)) > 0) ==1) 
useIndsEstY = which(1.0*(datesY >= startDate)*(datesY <= endDateEstimate)*(rowSums(!is.na(Yall)) > 0) ==1) 

useIndsForeX = which(1.0*(datesX >= startDate)*(datesX <= endDateForecast)*(rowSums(!is.na(Xall)) > 0) ==1) 
useIndsEstX = which(1.0*(datesX >= startDate)*(datesX <= endDateEstimate)*(rowSums(!is.na(Xall)) > 0) ==1)

Ytot = Yall[useIndsForeY,]; Y = Yall[useIndsEstY,]
TtotY = nrow(Ytot); TY = nrow(Y); forHorizY = TtotY - TY

Xtot = Xall[useIndsForeY,]; X = Xall[useIndsEstY,]
TtotX = nrow(Xtot); TX = nrow(X); forHorizX = TtotX - TX

useIndsY = which(colSums(!is.na(Y)) > 0); tauY = tauAllY[useIndsY]; Y = Y[,useIndsY]; Ytot = Ytot[,useIndsY]
useIndsX = which(colSums(!is.na(X)) > 0); tauX = tauAllX[useIndsX]; X = X[,useIndsX]; Xtot = Xtot[,useIndsX]

# Subset the dates (based on NAs):
useIndsY = which(rowSums(!is.na(Ytot)) > 0); Ytot = Ytot[useIndsY,]
useIndsY = which(rowSums(!is.na(Y)) > 0); Y = Y[useIndsY,]

useIndsX = which(rowSums(!is.na(Xtot)) > 0); Xtot = Xtot[useIndsX,]
useIndsX = which(rowSums(!is.na(X)) > 0); X = X[useIndsX,]

tauObsY = (tauY - min(tauY))/(diff(range(tauY))); 
mObsY = length(tauObsY);  # number of total observations points

tauObsX = (tauX - min(tauX))/(diff(range(tauX))); 
mObsX = length(tauObsX); 

mtbarY = mObsY # For this application, should be fine
mtbarX = mObsX

# Obtain the evaluaton points, update Y, and store "joint" points (tauAll)
Yobs = as.matrix(Y); Ytotobs = as.matrix(Ytot)
evalLs = getEvalPoints(as.matrix(Y), tauObsY, mEval = mObsY); Y = evalLs$Yall; tauAllY = evalLs$tauAll; mAllY = evalLs$mAll; tauEvalY = evalLs$tauEval; mEvalY = length(tauEvalY)
Ytot = getEvalPoints(as.matrix(Ytot), tauObsY, mEval = mObsY)$Yall; 
tauEvalY0 = tauEvalY*diff(range(tauY)) + min(tauY) # on the original scale

Xobs = as.matrix(X); Xtotobs = as.matrix(Xtot)
evalLs = getEvalPoints(as.matrix(X), tauObsX, mEval = mObsX); X = evalLs$Yall; tauAllX = evalLs$tauAll; mAllX = evalLs$mAll; tauEvalX = evalLs$tauEval; mEvalX = length(tauEvalX)
Xtot = getEvalPoints(as.matrix(Xtot), tauObsX, mEval = mObsX)$Yall; 
tauEvalX0 = tauEvalX*diff(range(tauX)) + min(tauX) # on the original scale

tauEvalY = tauAllY = (tauEvalY0 -min(tauY))/diff(range(tauY))
tauEvalX = tauAllX = (tauEvalX0 -min(tauX))/diff(range(tauX))

# Evaluate at these points:
tau_starY = sort(union(tauEvalY, seq(0, 1, length.out = 50))); 
tau_starX = sort(union(tauEvalX, seq(0, 1, length.out = 50))); 
...
sampleKappas = FALSE   
mtbar = mtbarY+mtbarX
psiInfo = getPsiInfo(c(tauEvalY,tauEvalX), m = mtbar, pMax = pMax)
B0starY = getLowRankTPS(tau_starY, m = sum(colMeans(!is.na(Y)) > 0)); B0Y = B0starY[match(tauAllY, tau_starY),]; 
B0starX = getLowRankTPS(tau_starX, m = sum(colMeans(!is.na(X)) > 0)); B0X = B0starX[match(tauAllX, tau_starX),];
...
muIntRepY = tcrossprod(rep(1, TY), muIntY) # For easy comparisons w/ mu (below)
...
muIntRepX = tcrossprod(rep(1, TX), muIntX) # For easy comparisons w/ mu (below)
...
alphaY = smoothMuInit( Y - muIntRepY, tauAllY, tauEvalY); 
muTotY = muIntRepY; muTotY[,match(tauEvalY, tauAllY)] = muTotY[,match(tauEvalY, tauAllY)] + alphaY # estimates of E[Y_t]

alphaX = smoothMuInit( X - muIntRepX, tauAllX, tauEvalX); 
muTotX = muIntRepX; muTotX[,match(tauEvalX, tauAllX)] = muTotX[,match(tauEvalX, tauAllX)] + alphaX # estimates of E[X_t]
...
###########################################################
evoResids = alpha; for(nj in (1:pMax)) evoResids[(pMax+1):TY,] = evoResids[(pMax+1):TY,] - tcrossprod(alpha[((pMax+1):TY - nj),], Psi_matrix[,p.inds[nj]:(p.inds[nj+1]- 1)])
# Initialize the evolution error covariance (+ associated parameters)
fdlmIn = initFDLM(c(tauEvalY,tauEvalX), mtbar, evoResids, fixSigma_eta, tolFPC, sampleFLCs, Je = NULL, c(tau_starY,tau_starX)); covParams = fdlmIn$covParams; fdlmParams = fdlmIn$fdlmParams
...
muFore =  muForeH = matrix(0, nr=TtotY-TY, nc=(mAll))
...
############################################################
# postMuTotFore = postMuTotForeH = array(0, c((nsims-burnin)/thin, TtotY - TY, mAll))
...
for(nsi in 1:nsims){
  ...
  evoResids = alpha; for(aj in allj) evoResids[(pMax+1):TY,] = evoResids[(pMax+1):TY,] - tcrossprod(alpha[((pMax+1):TY - aj),], sj[aj]*Psi_matrix[,p.inds[aj]:(p.inds[aj+1] - 1)])
  ...
  Model$H[,,1] = diag(sigma_nu^2, mAllY);  Model$T[1:(mAll),1:(mAll),1] = Tmat;  Model$Q[1:(mAll),1:(mAll),1] = Model$P1[1:(mAll),1:(mAll)] =  Keps
  
  #Sample alpha_t's
  alpha = simulateSSM(Model, "states", nsim = 1, antithetics=FALSE, filtered=FALSE)[,1:(mAll),1]
  
  # Same with the forecast data:
  ModelFore = ModelsFore[[jstar]]; ModelFore$y = Ytot - tcrossprod(rep(1, TtotY), muIntY)
  ModelFore$H[,,1] = diag(sigma_nu^2, mAllY); ModelFore$T[1:(mAll),1:(mAll),1] = Tmat;  ModelFore$Q[1:(mAll),1:(mAll),1] = ModelFore$P1[1:(mAll),1:(mAll)] = Keps
  ...
  muForeH = cbind(tcrossprod(rep(1, forHorizY), muIntY),tcrossprod(rep(1, forHorizX), muIntX)) +  tcrossprod(kfstemp$a[(TY+1):TtotY,], Ghm1)[,1:(mAll)]   
  for(ti in (TY+1):TtotY) muFore[ti - TY, ] = c(muIntY,muIntX) + kfstemp$a[ti,1:(mAll)] + crossprod(chol(kfstemp$P[1:(mAll),1:(mAll),ti]), rnorm(mEvalX+mEvalY)) 
  
  # Overall mean function:
  alphaY = alpha[,1:mAllY]  
  alphaX = alpha[,(mAllY+1):(mAll)]
  ...
  muIntRepY = tcrossprod(rep(1, TY), muIntY)
  # What we're really interested in: the non-centered parameter
  muTotY = muIntRepY; muTotY[,match(tauEvalY, tauAllY)] = muTotY[,match(tauEvalY, tauAllY)] + alphaY # estimates of Y_t
  ...
  muIntRepX = tcrossprod(rep(1, TX), muIntX)
  # What we're really interested in: the non-centered parameter
  muTotX = muIntRepX; muTotX[,match(tauEvalX, tauAllX)] = muTotX[,match(tauEvalX, tauAllX)] + alphaX # estimates of Y_t
...
diag(var_all) = 1.85*diag(var_all)
...
for(nsi in 1:S)
{
  ...
  Pd_far = prior_far(10^4,psiInfo,Sample_lambdaPsi[nsi],Sample_thetapsiscale[nsi], Sample_thetapsiS[nsi, ])
...
M = cbind(h_theta,Pd_total,pd_yt,value, deviation)
for(i in 1:T){
  alpha_t = mean_Gp%*%as.vector(c(alphaY[i,],alphaX[i,]))
  mean_yt = Z%*%mean_muInt + Z%*%alpha_t
  Sigma_tilde = (mean_Gp%*%diag(mAll)%*%t(mean_Gp)) + mean_Keps
  var_yt = diag(mean(Sample_sigma), mAllY) + Z%*%Sigma_tilde%*%t(Z)
  log_pd_yt[i] = -(0.5*mAllY*log(2*pi)) - (0.5*log(abs(det(var_yt)))) - 0.5*t(as.matrix(muTotY[i,]-mean_yt))%*%inv(var_yt)%*%(as.matrix(muTotY[i,]-mean_yt))
}
