...
#nsims =1020; burnin = 500; thin = 1;	
#draws = (nsims-20)# MCMC parameters
############################################################
...
sampleKappas = FALSE   
mtbar = mtbarY
psiInfo = getPsiInfo(tauEvalY, m = mtbar, pMax = pMax)
...
# Useful index:
p.inds= seq(1, mEvalY*(pMax+1), by=mEvalY)   
...
persp(x = seq(0, 1, length.out = nrow(z)), y = seq(0, 1, length.out = ncol(z)), z, theta = 120, phi = 15,xlab = "days" ,ylab = "time", zlab = "Y", shade = 0.5)

sigma_nu = sqrt(sum(( Y - muTotY)^2, na.rm=TRUE)/sum(!is.na(Y)))

alpha= alphaY # Combine alpha_t's
# FAR kernel:
...
fdlmIn = initFDLM(tauEvalY, mtbar, evoResids, fixSigma_eta, tolFPC, sampleFLCs, Je = NULL, tau_starY); covParams = fdlmIn$covParams; fdlmParams = fdlmIn$fdlmParams
...
for(nsi in 1:nsims){
  ...
  muForeH = tcrossprod(rep(1, forHorizY), muIntY) +  tcrossprod(kfstemp$a[(TY+1):TtotY,], Ghm1)[,1:(mAll)]   
  for(ti in (TY+1):TtotY) muFore[ti - TY, ] = muIntY + kfstemp$a[ti,1:(mAll)] + crossprod(chol(kfstemp$P[1:(mAll),1:(mAll),ti]), rnorm(mEvalY)) 
  ...
  sum_muInt = sum_muInt + muIntSampY$muInt
...
Sample_all = NULL
Sample_all = cbind(Sample_thetaIntY,Sample_sigma,Sample_sigmaeta,Sample_sigmaj2,Sample_lambdaphi)
...
for(nsi in 1:S)
{
  ...
  Pd_total[nsi] = PdSigma_nu+Pd_et+Pd_flc+Pd_muY+Pd_far
  log_pd_yt = vector(length = mAll)
  SigmaTbyT = diag(mAll)
  for (i in 1:T) {   
    alpha_t = Sample_far[[nsi]]$Gp%*%as.matrix(alphaY[i,])
    mean_yt = Z%*%Sample_muIntY[[nsi]]$muInt + Z%*%alpha_t
    ...
    vt = muTotY[i,] - Z%*%Sample_muIntY[[nsi]]$muInt - Z%*%alpha_t
...
for(i in 1:T){
  alpha_t = mean_Gp%*%as.vector(alphaY[i,])
