...
Kinvfun = function(PhiMat, Sigma, sigma_eta, SigmaInv = NULL) {
  if(is.numeric(Sigma)){  # When Sigma is diagoinal, compute inverse explicitly
    Sigmatilde = diag(sigma_eta^-2/(Sigma^-1 + sigma_eta^-2))
...
# Simulate FAR(p) data 
############################################################
simDataP = function(Ttot = 50, kernelType , errorDist = 'smoothGP', sampDesign = c(mobs=30, mobsAvg=30, randLocation = FALSE), psiNorm , obsNoiseToEvoNoise = 2/10){
  ...
  m = 50; taugrid = seq(0, 1, length.out=m)		 
  ...
  for(j in 1:p){
    if(kernelType[j] == 'bg')      krnl0[[j]] = function(tau, u){sigmax = .3; sigmaz = .4; 0.75/(pi*sigmax*sigmaz)*exp(-(tau -.5)^2/sigmax^2 - (u-.3)^2/sigmaz^2) + 0.45/(pi*sigmax*sigmaz)*exp(-(tau -.7)^2/sigmax^2 - (u-.8)^2/sigmaz^2)}
  ...
  plot(tau, Y[1,],ylim = c(-0.08,0.04),type ='l')
  for(i in 2:Ttot){lines(tau,Y[i,],ylim = c(-0.08,0.04),type ='l')}
  
  plot(taugrid, omega[1,],ylim = c(-0.03,0.03),type ='l')
  for(i in 2:Ttot){lines(taugrid,omega[i,],ylim = c(-0.03,0.03),type ='l')}
  return(list(Y = Y, muTotAll=muTotAll, muTotOracle = muTotOracle))
}

#simulate Y and X
simData = function(Ttot , kernelType, errorDist , sampDesign, psiNorm , obsNoiseToEvoNoise, sigmax, sigmaz){
  ...
  krnl = krnl0 = krnlAll = vector('list', p); norm2 = numeric(p)
  for(j in 1:p){
    if(kernelType[j] == 'bg')      krnl0[[j]] = function(tau, u){0.75/(pi*sigmax*sigmaz)*exp(-(tau -.2)^2/sigmax^2 - (u-.3)^2/sigmaz^2) + 0.45/(pi*sigmax*sigmaz)*exp(-(tau -.7)^2/sigmax^2 - (u-.8)^2/sigmaz^2)}
    if(kernelType[j] == 'linTau')  krnl0[[j]] = function(tau,u){ tau + 0*u }		# Linear (tau)
    if(kernelType[j] == 'linU')    krnl0[[j]] = function(tau,u){ 0*tau + u }		# Linear (u)
    if(kernelType[j] == 'usin')    krnl0[[j]] = function(tau, u){u*sin(2*pi*tau)}		# from McLean et al.
  ...
  # plot(taugrid, omega[1,],ylim = c(-0.03,0.03),type ='l')
  # for(i in 2:Ttot){lines(taugrid,omega[i,],ylim = c(-0.03,0.03),type ='l')}
  return(list(Y = Y, muTotAll=muTotAll, muTotOracle = muTotOracle, muInt = muInt))
}
############################################################
...
############################################################
fillNA = function(Y, tauAll, linearInterp=FALSE){
  Y0 = Y
  if(sum(is.na(Y0) > 0)){
...
forecastDRA = function(yi, tau0, yiInterp, h, useDiagonalQ = TRUE, includeIntercept = TRUE, estimateLambda = TRUE){
  ...
  update_model = function(pars, model){
    ...
    #diag(model$P1) = 0 ## added by me mine
    model
  }
...
############################################################
sampleMuInt = function(Yres, thetaInt,B0, sigma_nu, muIntInfo, coefPriorPrec){
  # Unpack: 
...
############################################################
initFARkernel = function(psiInfo, mu1, mu2, pMax = 1, AscalePrec =  10^-6){
  
  mEval = ncol(mu1); T1 = nrow(mu1) ; T2 = nrow(mu2)
  cov_mat = cov(mu1[(pMax+1):T1,],mu2[((pMax+1):T1 - 1),])
  # Unpack psiInfo:
  ...
  theta_psi = numeric(J1*J2*pMax); lambdaPsi = rep(10, pMax) # Create the vectors for storage
  ...
  if(pMax > 1){
    for(i1 in 1:pMax){
      ...
      HtKinvMu[i1Block] = matrix(tcrossprod(crossprod(B1, cov_mat), B2tQ))
      priorPrec[i1Block, i1Block] = lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
      for(i2 in i1:pMax){
        i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(cov_mat, B2tQ), crossprod(B1))
        HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
      }
    }
  } else { # pMax = 1
    priorPrec = lambdaPsi*(OmegaPsi + kappa_s*OmegaNorm)
    HtKinvH = kronecker(B2tQ%*%tcrossprod(cov_mat, B2tQ), crossprod(B1))
    HtKinvMu = matrix(tcrossprod(crossprod(B1, cov_mat), B2tQ))
  }
...
# Initialize the FAR kernel
############################################################
initFARkernel2 = function(psiInfo, mu1, mu2, pMax = 1, AscalePrec =  10^-6){
  
  mEval = ncol(mu1); T = nrow(mu1)
  ...
  if(pMax > 1){
    for(i1 in 1:pMax){
      i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
      HtKinvMu[i1Block] = matrix(tcrossprod(crossprod(B1, crossprod(mu1[(pMax+1):T,],mu2[((pMax+1):T - i1),])), B2tQ))
      priorPrec[i1Block, i1Block] = lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
      for(i2 in i1:pMax){
        i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(crossprod(mu1[((pMax+1):T - i1),], mu1[((pMax+1):T - i2),]), B2tQ), crossprod(B1))
        HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
      }
    }
  } else { # pMax = 1
    priorPrec = lambdaPsi*(OmegaPsi + kappa_s*OmegaNorm)
    HtKinvH = kronecker(B2tQ%*%tcrossprod(crossprod(mu1[((pMax+1):T - 1),]), B2tQ), crossprod(B1))
    HtKinvMu = matrix(tcrossprod(crossprod(B1, crossprod(mu1[(pMax+1):T,],mu2[((pMax+1):T - 1),])), B2tQ))
  }
  # Compute initial estimate:
  chUphiInv = chol(priorPrec + HtKinvH); 
  theta_psi = backsolve(chUphiInv,forwardsolve(t(chUphiInv), HtKinvMu))		
...
initFARkernel3 = function(psiInfo, mu, pMax = 1, AscalePrec =  10^-6){
  ...
  if(pMax > 1){
    for(i1 in 1:pMax){
      i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
      HtKinvMu[i1Block] = matrix(tcrossprod(crossprod(B1, mu), B2tQ))
      priorPrec[i1Block, i1Block] = lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
      for(i2 in i1:pMax){
        i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(mu, B2tQ), crossprod(B1))
        HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
      }
    }
  } else { # pMax = 1
    priorPrec = lambdaPsi*(OmegaPsi + kappa_s*OmegaNorm)
    HtKinvH = kronecker(B2tQ%*%tcrossprod(mu, B2tQ), crossprod(B1))
    HtKinvMu = matrix(tcrossprod(crossprod(B1, mu), B2tQ))
  }
  # Compute initial estimate:
  chUphiInv = chol(priorPrec + HtKinvH); 
  theta_psi = backsolve(chUphiInv,forwardsolve(t(chUphiInv), HtKinvMu))		
...
initFARkernel4 = function(psiInfo, muA, muH, pMax = 1, AscalePrec =  10^-6){
  
  mEval = ncol(muA); T = nrow(muA)
  ...
  if(pMax > 1){
    for(i1 in 1:pMax){
      i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
      HtKinvMu[i1Block] = matrix(tcrossprod(crossprod(B1, muA), B2tQ))
      priorPrec[i1Block, i1Block] = lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
      for(i2 in i1:pMax){
        i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(muH, B2tQ), crossprod(B1))
        HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
      }
    }
  } else { # pMax = 1
    priorPrec = lambdaPsi*(OmegaPsi + kappa_s*OmegaNorm)
    HtKinvH = kronecker(B2tQ%*%tcrossprod(muH, B2tQ), crossprod(B1))
    HtKinvMu = matrix(tcrossprod(crossprod(B1, muA), B2tQ))
  }
  # Compute initial estimate:
  chUphiInv = chol(priorPrec + HtKinvH); 
  theta_psi = backsolve(chUphiInv,forwardsolve(t(chUphiInv), HtKinvMu))		
...
initXi = function(PhiMat, Bphi, Jkl, Je){
  ...
  xi[,1] = chol2inv(cholFac)%*%crossprod(Bphi, PhiMat[,1]); xi[,1] = xi[,1]/as.vector(sqrt(crossprod(xi[,1], Jkl%*%xi[,1])))
  ...
  for(j in 2:Je){
    ...
    xi[,j] = xi[,j]/as.vector(sqrt(crossprod(xi[,j], Jkl)%*%xi[,j]))
...
initDLMs = function(Y, Gp, pMax=1, Ytot = NULL){
  ...
  for(j in 1:pMax){
    ...
    Zp = array(0, c(mAll1, mAll1*j, 1)); Zp[1:mAll1, 1:mAll1, 1] = diag(mAll1)
    # Evolution matrix:
    if(j > 1){
    ...
    Model = SSModel(Y~-1+SSMcustom1(Z = Zp, T = Gpdlm, Q = array(diag(mAll*j), c(mAll*j,mAll*j,1))))
...
initDLMs1 = function(Y,Gp, pMax=1, Ytot = NULL){
  mAll = nrow(Gp)
  Models = ModelsFore = vector("list", pMax) # Storage
  for(j in 1:pMax){
    # Observation matrix (mostly zeros)
...
initDLMs2 = function(Y, Gp, pMax=1, Ytot = NULL){
  mAll = nrow(Gp)/2
  Models = ModelsFore = vector("list", pMax) # Storage
  jstar = 2
  for(j in 1:pMax){
    ...
    # Initialize the model:
    Model = SSModel(Y~-1+SSMcustom(Z = Zp, T = Gpdlm, Q = array(diag(mAll*j*jstar), c(mAll*j*jstar,mAll*j*jstar,1))))
    Model$R[-(1:mAll),-(1:mAll),1] = 0; Models[[j]] = Model; 
...
initDLMs3 = function(Y, Gp, pMax=1, Ytot = NULL){
  mAll = nrow(Gp)
  mAll1 = ncol(Y)
  Models = ModelsFore = vector("list", pMax) # Storage
  for(j in 1:pMax){
    # Observation matrix (mostly zeros)
    Zp = array(0, c(mAll1, mAll*j, 1)); Zp[1:mAll1, 1:mAll1, 1] = diag(mAll1)
    # Evolution matrix:
    if(j > 1){
      Gpdlm =  array(0, c(mAll*j, mAll*j, 1)); Gpdlm[,,1] = rbind(Gp[,1:(mAll*j)], cbind(diag(mAll*(j-1)), matrix(0, nr=mAll*(j-1), nc=mAll)))
    } else Gpdlm =  array(0, c(mAll*j, mAll*j, 1))
    index = 1:mAll1
    # Initialize the model:
    Model = SSModel(Y~-1+SSMcustom(Z = Zp, T = Gpdlm, Q = array(diag(mAll*j), c(mAll*j,mAll*j,1)), index = index))
    Model$R[-(1:mAll),-(1:mAll),1] = 0; Models[[j]] = Model; 
...
############################################################
sampleFARkernel2 = function(Gp, Gp1, psiInfo, mu1, mu2 , KepsInv, sj, farParams, pMax=1, sampleKappas = TRUE){
  ...
  # Vectors for storage
  HtKinvH = quadLike = priorPrec = diag(J1*J2*pMax); HtKinvMu = linLike = numeric(J1*J2*pMax)
  #Step 3(a) of MCMC sampling
  
  for(i1 in 1:pMax){
    i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
    if(sj[i1]==1){
      HtKinvMu[i1Block] = linLike[i1Block] = matrix(tcrossprod(crossprod(B1, KepsInv%*%cov_mat), B2tQ))
    ...
    for(i2 in i1:pMax){
      i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
      if(sj[i1]*sj[i2] == 1){
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(cov_mat, B2tQ), crossprod(B1, KepsInv)%*%B1)
  ...
  for(j in 1:pMax){
    ...
    quadLike = kronecker(B2tQ%*%tcrossprod(cov_mat, B2tQ), crossprod(B1, KepsInv)%*%B1)
    notj = (1:pMax)[-j]; notj = notj[which(sj[notj] == 1)]; muResj = mu1; for(nj in notj) muResj[(pMax+1):T1,] = muResj[(pMax+1):T1,] - tcrossprod(mu1[((pMax+1):T1 - nj),], sj[nj]*Gp[,p.inds[nj]:(p.inds[nj+1] - 1)])
    linLike = matrix(tcrossprod(crossprod(B1, KepsInv%*%crossprod(muResj[(pMax+1):T1,],mu2[((pMax+1):T2 - j),])), B2tQ))
    
    #  AscalePrec = 10
    A =tcrossprod(tcrossprod(theta_psi[jBlock], quadLike),theta_psi[jBlock])
    if(is.nan(A)) {AscaleSD = 1/sqrt(AscalePrec)}else{AscaleSD = 1/sqrt(abs(tcrossprod(tcrossprod(theta_psi[jBlock], quadLike),theta_psi[jBlock]) + AscalePrec))}
    uphiS = crossprod(theta_psi[jBlock], linLike)
    theta_psi_scale[j] = rnorm(1, mean=AscaleSD^2*uphiS, sd= AscaleSD)
    #  theta_psi_scale[j] = 0.1
    ...
    shapej = 0.5 + J1*J2/2; ratej = 0.5 + crossprod(theta_psi[jBlock], (OmegaPsi + kappa_s[j]*OmegaNorm))%*%theta_psi[jBlock]/2
...
############################################################
sampleFARkernel3 = function(Gp, Gp1, psiInfo, mu , KepsInv, sj, farParams, pMax=1, sampleKappas = TRUE){
  
  mEval = ncol(mu); T1 = nrow(mu);
  # Unpack psiInfo and farParams:
  B1 = psiInfo$B1; B2 = psiInfo$B2; OmegaPsi = psiInfo$OmegaPsi; OmegaNorm = psiInfo$OmegaNorm; B2tQ = psiInfo$B2tQ; 
  theta_psi_scale = farParams$theta_psi_scale; lambdaPsi = farParams$lambdaPsi; AscalePrec = farParams$AscalePrec; kappa_s = farParams$kappa_s; logkappa_s = farParams$logkappa_s
  ...
  for(i1 in 1:pMax){
    i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
    if(sj[i1]==1){
      HtKinvMu[i1Block] = linLike[i1Block] = matrix(tcrossprod(crossprod(B1, KepsInv%*%mu), B2tQ))
      HtKinvMu[i1Block] = theta_psi_scale[i1]*HtKinvMu[i1Block] # Gelman prior
    } else {HtKinvMu[i1Block] = linLike[i1Block] = 0}
    priorPrec[i1Block, i1Block] =  lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
    for(i2 in i1:pMax){
      i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
      if(sj[i1]*sj[i2] == 1){
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(mu, B2tQ), crossprod(B1, KepsInv)%*%B1)
        quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = Blocki1i2
        Blocki1i2 = theta_psi_scale[i1]*theta_psi_scale[i2]*Blocki1i2 # Gelman prior
      } else {Blocki1i2 = quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = 0}
      HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
  ...
  for(j in 1:pMax){
    ...
    # Sample the scale parameter:
    quadLike = kronecker(B2tQ%*%tcrossprod(mu, B2tQ), crossprod(B1, KepsInv)%*%B1)
    notj = (1:pMax)[-j]; notj = notj[which(sj[notj] == 1)]; muResj = mu; for(nj in notj) muResj[(pMax+1):T1,] = muResj[(pMax+1):T1,] - tcrossprod(mu1[((pMax+1):T1 - nj),], sj[nj]*Gp[,p.inds[nj]:(p.inds[nj+1] - 1)])
    linLike = matrix(tcrossprod(crossprod(B1, KepsInv%*%crossprod(muResj[(pMax+1):T1,],mu[((pMax+1):T1 - j),])), B2tQ))
...
sampleFARkernel4 = function(Gp, Gp1, psiInfo, muA, muH , KepsInv, sj, farParams, pMax=1, sampleKappas = TRUE){
  
  mEval = ncol(muA); T1 = nrow(muA);
  # Unpack psiInfo and farParams:
  B1 = psiInfo$B1; B2 = psiInfo$B2; OmegaPsi = psiInfo$OmegaPsi; OmegaNorm = psiInfo$OmegaNorm; B2tQ = psiInfo$B2tQ; 
  theta_psi_scale = farParams$theta_psi_scale; lambdaPsi = farParams$lambdaPsi; AscalePrec = farParams$AscalePrec; kappa_s = farParams$kappa_s; logkappa_s = farParams$logkappa_s
  ...
  for(i1 in 1:pMax){
    i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
    if(sj[i1]==1){
      HtKinvMu[i1Block] = linLike[i1Block] = matrix(tcrossprod(crossprod(B1, KepsInv%*%muA), B2tQ))
      HtKinvMu[i1Block] = theta_psi_scale[i1]*HtKinvMu[i1Block] # Gelman prior
    } else {HtKinvMu[i1Block] = linLike[i1Block] = 0}
    priorPrec[i1Block, i1Block] =  lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
    for(i2 in i1:pMax){
      i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
      if(sj[i1]*sj[i2] == 1){
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(muH, B2tQ), crossprod(B1, KepsInv)%*%B1)
        quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = Blocki1i2
        Blocki1i2 = theta_psi_scale[i1]*theta_psi_scale[i2]*Blocki1i2 # Gelman prior
      } else {Blocki1i2 = quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = 0}
      HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
  ...
  for(j in 1:pMax){
    ...
    # Sample the scale parameter:
    quadLike = kronecker(B2tQ%*%tcrossprod(muH, B2tQ), crossprod(B1, KepsInv)%*%B1)
    notj = (1:pMax)[-j]; notj = notj[which(sj[notj] == 1)]; muResj = muH; for(nj in notj) muResj[(pMax+1):T1,] = muResj[(pMax+1):T1,] - tcrossprod(muH[((pMax+1):T1 - nj),], sj[nj]*Gp[,p.inds[nj]:(p.inds[nj+1] - 1)])
    linLike = matrix(tcrossprod(crossprod(B1, KepsInv%*%crossprod(muResj[(pMax+1):T1,],muH[((pMax+1):T1 - j),])), B2tQ))
...
# Sample FAR kernel paramaters
############################################################
sampleFARkernel = function(Gp, Gp1, psiInfo, mu1,mu2, KepsInv, sj, farParams, pMax=1, sampleKappas = TRUE){
  ...
  for(i1 in 1:pMax){
    i1Block = blockSeq[i1]:(blockSeq[i1+1] - 1)
    if(sj[i1]==1){
      HtKinvMu[i1Block] = linLike[i1Block] = matrix(tcrossprod(crossprod(B1, KepsInv%*%crossprod(mu1[(pMax+1):T,],mu2[((pMax+1):T - i1),])), B2tQ))
      HtKinvMu[i1Block] = theta_psi_scale[i1]*HtKinvMu[i1Block] # Gelman prior
    } else {HtKinvMu[i1Block] = linLike[i1Block] = 0}
    priorPrec[i1Block, i1Block] =  lambdaPsi[i1]*(OmegaPsi + kappa_s[i1]*OmegaNorm)
    for(i2 in i1:pMax){
      i2Block = blockSeq[i2]:(blockSeq[i2+1] - 1)
      if(sj[i1]*sj[i2] == 1){
        Blocki1i2 = kronecker(B2tQ%*%tcrossprod(crossprod(mu1[((pMax+1):T - i1),], mu1[((pMax+1):T - i2),]), B2tQ), crossprod(B1, KepsInv)%*%B1)
        quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = Blocki1i2
        Blocki1i2 = theta_psi_scale[i1]*theta_psi_scale[i2]*Blocki1i2 # Gelman prior
      } else {Blocki1i2 = quadLike[i1Block, i2Block] = quadLike[i2Block, i1Block] = 0}
      HtKinvH[i1Block, i2Block] = Blocki1i2; HtKinvH[i2Block, i1Block] = t(Blocki1i2)
  ...
  for(j in 1:pMax){
    ...
    # Sample the scale parameter:
    quadLike = kronecker(B2tQ%*%tcrossprod(crossprod(mu1[((pMax+1):T - j),]), B2tQ), crossprod(B1, KepsInv)%*%B1)
    notj = (1:pMax)[-j]; notj = notj[which(sj[notj] == 1)]; muResj = mu1; for(nj in notj) muResj[(pMax+1):T,] = muResj[(pMax+1):T,] - tcrossprod(mu1[((pMax+1):T - nj),], sj[nj]*Gp[,p.inds[nj]:(p.inds[nj+1] - 1)])
    linLike = matrix(tcrossprod(crossprod(B1, KepsInv%*%crossprod(muResj[(pMax+1):T,],mu1[((pMax+1):T - j),])), B2tQ))
...
############################################################
getPsiInfo = function(tauEval, m , pMax = 1){
  
  # Trapezoidal weights for integral approx:
  Qmat = .5*(diag(c(0, diff(tauEval))) + diag(c(diff(tauEval), 0)))
  
  # Tensor product of B-splines:
  psiSplineInfo = getBivariateBasis(tauEval, m ); B1 = psiSplineInfo$B1; B2 = psiSplineInfo$B2; OmegaPsi = psiSplineInfo$OmegaPsi; OmegaNorm = psiSplineInfo$OmegaNorm; 
...
SSMcustom1 = function (Z, T, R, Q, a1, a2, P1, P1inf, index, n = 1, state_names = NULL) 
{
  if (missing(index)) 
    index <- 1
  p <- dim(Y)[2]
  if (length(Z) == 1 && p == 1) {
    ...
    if ((length(Z) == 1) || !(dim(Z)[1] == p) || !dim(Z)[3] %in% 
        c(1, NA, n)) 
      stop("Misspecified Z,  time series, m is the number of states.")
    m <- dim(Z)[2]
    dim(Z) <- c(p, m, (n - 1) * (max(dim(Z)[3], 0, na.rm = TRUE) > 
                                   1) + 1)
  }
  if (length(T) == 1 && m == 1) {
    dim(T) <- c(1, 1, 1)
  ...
  if (length(Q) == 1) {
    ...
    if (!identical(dim(Q)[1], dim(Q)[2]) || dim(Q)[1] > m || 
        !dim(Q)[3] %in% c(1, NA, n)) 
    ...
    k <- dim(Q)[1]
    dim(Q) <- c(k, k, (n - 1) * (max(dim(Q)[3], 0, na.rm = TRUE) > 
                                   1) + 1)
  }
  if (missing(R)) {
    ...
    if (all(c(length(R), k, m) == 1)) {
      ...
      if ((length(R) == 1) || !(dim(R)[1] == m) || dim(R)[2] != 
          k || !dim(R)[3] %in% c(1, NA, n)) 
      ...
      dim(R) <- c(m, k, (n - 1) * (max(dim(R)[3], 0, na.rm = TRUE) > 
                                     1) + 1)
    }
  }
  if (missing(a1)) {
  ...
  list(index = index, m = m, k = k, p = p, n = n, Z = Z, T = T, 
       R = R, Q = Q, a1 = a1, P1 = P1, P1inf = P1inf, tvz = dim(Z)[3] > 
         1, tvt = dim(T)[3] > 1, tvr = dim(R)[3] > 1, tvq = dim(Q)[3] > 
         1, state_names = state_names)
}
...
#Prior Distributions
###########################################################################
prior_phij = function(lambdaphi, xi)
{
  # J = length(lambdaphi); K = nrow(xi)
  # pd_lambdaphi = dunif(lambdaphi^(-0.5), min = 0, max = 10^4)
  # zeta_var = diag(c(10^8,10^8, rep(lambdaphi[J]^(-1),K-2)))
  # pd_xi_vec = dmvnorm(t(xi), mean = rep(0,K),sigma = zeta_var)
  # pd_xi = prod(pd_xi_vec)  ##
  # pd_lambdaphi[1]*(max(pd_xi_vec))
  
  M = nrow(xi); J = ncol(xi)
  log_pd_xi = vector(length = J)
  for(i in 1:J)
  {
    var_xi = diag(c(10^4,10^4, rep(lambdaphi[i]^(-1),M-2)))
    log_pd_xi[i] = -(0.5*log(2*pi)) - (0.5*log(abs(det(var_xi)))) - (0.5*(t(as.matrix(xi[,i]))%*%inv(var_xi)%*%(as.matrix(xi[,i]))))
  }
  sum(log_pd_xi)-M*log(10)
}

prior_mu = function(thetaInt)
{
  J = length(thetaInt) 
  lambdaphi = 1/(runif(1,0,10^4)^2)
  #pd_lambdaphi = dunif(lambdaphi^(-0.5), min = 0, max = 10^4)
  var_mu = diag(c(10^8,10^8, rep(lambdaphi^(-1),(J-2))))
  # pd_thetaInt = dmvnorm(t(thetaInt), mean = rep(0,J+2),sigma = var)
  pd_thetaInt = -(0.5*J*log(2*pi)) - (0.5*log(abs(det(var_mu)))) - (0.5*t(as.matrix(thetaInt))%*%inv(var_mu)%*%(as.matrix(thetaInt)))
  #pd_lambdaphi*pd_thetaInt
  pd_thetaInt
}

prior_far = function(C,psiInfo, lambdaPsi, thetapsiscale, thetapsiS)
{
  priorPrec  = matrix(nrow = nrow(psiInfo$OmegaPsi), ncol = ncol(psiInfo$OmegaPsi))
  priorPrec =  lambdaPsi*(psiInfo$OmegaPsi+ psiInfo$OmegaNorm)
  # pd_lambdaPsi = dgamma(lambdaPsi ,0.5,0.5)  ##
  pd_thetascale = -(0.5*log(2*pi)) - (3*log(10)) - (0.5*10^-6*thetapsiscale)
  lambdaPsi = (thetapsiscale^-2)*lambdaPsi
  var_theta = (lambdaPsi^-1)*(inv(priorPrec))*C
  L = length(thetapsiS)
  #pd_thetapsi= dmvnorm(thetapsiS,rep(0,nrow(priorPrec)),var_theta)
  pd_theta = -(0.5*L*log(2*pi)) - (0.5*(t(as.matrix(thetapsiS))%*%inv(var_theta)%*%(as.matrix(thetapsiS))))
  #pd_lambdaPsi*pd_thetapsi
  pd_thetascale + pd_theta
}

prior_ejt = function(sigmaeta, sigmaj2)
{
  J = length(sigmaj2)
  #PdSigmaj2 = vector(length = J)
  #pd_Sigma_eta = dgamma(sigma_eta^(-2),10^(-3),10^(-3))
  # PdSigmaj2[J] = dgamma(sigmaj2[J]^(-1),10^(-3),10^(-3))
  # for(i in 1:(J-1))
  # {PdSigmaj2[(J-i)] = dunif(sigmaj2[(J-i)],min= 0,max = sigmaj2[(J-i+1)])}
  # pd_sigmaj2 = prod(PdSigmaj2)
  # Sigma_e = diag(sigmaj2,J,J)
  # L = nrow(ejt)
  # Pdejt_vec = vector(length = L)
  # for(i in 1:L)
  # {Pdejt_vec[i] = dmvnorm(ejt[i,] ,mean= c(0,0,0,0), Sigma_e)}
  # pd_ejt = prod(Pdejt_vec)
  
  log_pd_sigmaeta = ((10^-3 -1)*log(sigmaeta^(-2)))-(log(gamma(10^-3)))-((10^-3)*log(10^-3))-((10^-3)*sigmaeta^(-2))
  log_pd_sigmaj2 = ((10^-3 -1)*log(sigmaj2[1]^(-2)))-(log(gamma(10^-3)))-((10^-3)*log(10^-3))-((10^-3)*sigmaj2[1]^(-2))
  log_pd_sigmaeta+log_pd_sigmaj2-12*log(10)
  
  #pd_Sigma_eta*PdSigmaj2[J]
}

mean_vector = function(Sample, nsims)
{
  J = ncol(Sample)
  mean_vec = vector(length = J)
  for(i in 1:J)
  {mean_vec[i] = mean(Sample[1:nsims,i])}
  mean_vec
}

mean_matrix = function(Sample, nsims)
{
  M = length(Sample[1, ,1])
  N = length(Sample[1,1, ])
  mean_mat = matrix(0,M,N)
  for(m in 1:M)
  { for(n in 1:N)
  {mean_mat[m,n] = mean(Sample[1:nsims,m,n])}
  }
  mean_mat
}

log_prior_mu = function(Sample_thetaInt)
{
  nsims = nrow(Sample_thetaInt)
  K = ncol(Sample_thetaInt)
  mean_mu = colMeans(Sample_thetaInt)
  var_mu = cov(Sample_thetaInt)
  sum_h = 0 
  for(j in 1:nsims) sum_h = sum_h + t(as.matrix(Sample_thetaInt[j,]-mean_mu))%*%inv(var_mu)%*%(as.matrix(Sample_thetaInt[j,]-mean_mu))
  log_pd_mu = -(0.5*K*nsims*log(2*pi)) - (0.5*nsims*log(abs(det(var_mu)))) - (0.5*sum_h)
  log_pd_mu-4*log(10)
}

log_prior_et = function(Sample_sigmaeta, Sample_sigmaj2)
{
  nsims = nrow(Sample_sigmaj2)
  J = ncol(Sample_sigmaj2)
  log_pd_sigmaeta = ((10^-3 -1)*sum(log(Sample_sigmaeta^(-2))))-(nsims*log(gamma(10^-3)))-((10^-3)*nsims*log(10^-3))-((10^-3)*sum(Sample_sigmaeta^(-2)))
  log_pd_sigmaj2 = ((10^-3 -1)*sum(log(Sample_sigmaj2[,J]^(-2))))-(nsims*log(gamma(10^-3)))-((10^-3)*nsims*log(10^-3))-((10^-3)*sum(Sample_sigmaj2[,J]^(-2)))
  log_pd_sigmaeta+log_pd_sigmaj2-12*log(10)
}

log_prior_phij = function(Sample_xi)
{
  nsims = length(Sample_xi[,1,1])
  M = length(Sample_xi[1, ,1])
  J = length(Sample_xi[1,1,])
  xi = matrix(0,nsims,M)
  log_pd_xi = vector(length = J)
  for(i in 1:J)
  {
    ...
    var_xi = cov(xi)
    sum_h = 0 
    for(j in 1:nsims) sum_h = sum_h + t(as.matrix(xi[j,]-mean_xi))%*%inv(var_xi)%*%(as.matrix(xi[j,]-mean_xi))
    log_pd_xi[i] = -(0.5*M*nsims*log(2*pi)) - (0.5*nsims*log(abs(det(var_xi)))) - (0.5*sum_h)
  }
  sum(log_pd_xi)-16*log(10)
}

log_prior_far = function(psiInfo,Sample_lambdaPsi,Sample_thetapsiscale, Sample_thetapsiS)
{
  nsims = length(Sample_thetapsiscale)
  L = ncol(Sample_thetapsiS)
  log_pd_thetascale = -(0.5*nsims*log(2*pi)) - (3*nsims*log(10)) - (0.5*10^-6*sum(Sample_thetapsiscale))
  priorPrec  = matrix(nrow = nrow(psiInfo$OmegaPsi), ncol = ncol(psiInfo$OmegaPsi))
  priorPrec =  mean(Sample_lambdaPsi)*(psiInfo$OmegaPsi + psiInfo$OmegaNorm)
  var_theta = (mean(Sample_lambdaPsi)^-1)*(inv(priorPrec))
  sum_h = 0 
  for(j in 1:nsims) sum_h = sum_h + t(as.matrix(Sample_thetapsiS[j,]))%*%inv(var_theta)%*%(as.matrix(Sample_thetapsiS[j,]))
  log_pd_theta = -(0.5*L*nsims*log(2*pi)) - (0.5*nsims*log(abs(det(var_theta)))) - (0.5*sum_h)
  
  log_pd_theta+log_pd_thetascale
}
